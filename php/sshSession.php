      ...
      if (isset($_SESSION['logged_in'])) {
        if ($_SESSION['logged_in'] == '1') {
          if(getAcclvl($_SESSION["username"]) == 1) {
            $f = [];
            exec("sudo /var/www/html/php_scripts/sshd.sh 2>&1", $f);
            foreach ($f as $line) {
              echo $line;
              echo "<br/>";
            }
          } else {
            error_log("**Visita a sessionSsh.php non autorizzata 1**: Logged_in: " . $_SESSION["logged_in"] . " Username: " . $_SESSION['username'] . " acc_lvl: " . getAcclvl($_SESSION["username"]) . " Ip: " . $_SERVER["REMOTE_ADDR"]);
            die("<h1>Non sei autorizzato a visitare questa pagina, questo incidente é stato registrato.<h1>");
          }
        } else {
          error_log("**Visita a sessionSsh.php non autorizzata 2**: Logged_in: " . $_SESSION["logged_in"] . " Username: NULL (Il login non é stato eseguito)" . " Ip: " . $_SERVER["REMOTE_ADDR"]);
          die("<h1>Non sei autorizzato a visitare questa pagina, questo incidente é stato registrato.<h1>");
        }
      } else {
        error_log("**Visita a sessionSsh.php non autorizzata 3**: Logged_in: NULL non é stato loggato Username: NULL non é stato loggato Ip: " . $_SERVER["REMOTE_ADDR"]);
        die("<h1>Non sei autorizzato a visitare questa pagina, questo incidente é stato registrato.<h1>");
      }
     ?>
  </body>
