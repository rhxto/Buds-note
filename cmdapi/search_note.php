<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($type == "search_note"){
      ...
      if ((!isset($_POST["note_id"]) || $_POST["note_id"] === "") || $_POST["note_id"] === "undefined") {
        $note_id = NULL;
      } else {
        $note_id = test_input($_POST["note_id"]);
        if ($_POST["note_id"] != $note_id) {
          //echo 'nonAN';
          $result = [
            "status"=>"fail",
            "code"=>"56_title_not_valid"
    ...
    if ($type == "search_note"  && empty($result["status"])) {
      ...
      $response = searchNote(connectDb(), $title, $note_id, NULL, $user, $subj, $years_array, $dept, $datefrom, $dateto, $orderby, $order);
